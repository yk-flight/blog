...
import com.zrkizzy.server.template.MessageSenderTemplate;
...
public class EmailMessageSender extends MessageSenderTemplate {
    @Autowired
    ...
    public void sendMessage(EmailMessageDTO emailMessageDTO) {
        // 获取存储到Redis中的验证码
        String verifyCode = verifyCode(EMAIL, emailMessageDTO.getSenderTo());
        // 判断邮件发送的类型来定义邮件内容
        String content = switch (emailMessageDTO.getType()) {
            // 修改密码验证码
        ...
        // 发件人
        simpleMailMessage.setFrom(emailMessageDTO.getSender());
        // 收件人
        simpleMailMessage.setTo(emailMessageDTO.getSenderTo());
        // 邮件主题
